...
int LineHeights[6] = { 12, 0, 0, 38, 0, 50 };


std::array<std::array<SDL_Color, 256>, 5> FontColorTables;
...
void InitText()
{
	for (int i = 0; i < 6; i++) {
		char path[32];
		sprintf(path, "fonts\\s%i-00.pcx", FontSizes[i]);
...
void UpdateFontColors()
{
	for (int i = 240; i < 255; i++)
		FontColorTables[ColorWhite][i] = system_palette[i - 3];
	for (int i = 240; i < 255; i++)
		FontColorTables[ColorBlue][i] = system_palette[i - 65];
	for (int i = 240; i < 255; i++)
		FontColorTables[ColorRed][i] = system_palette[i - 19];
	for (int i = 240; i < 255; i++)
		FontColorTables[ColorGold][i] = system_palette[i - 48];
	for (int i = 240; i < 255; i++)
...
uint16_t DrawString(const Surface &out, string_view text, const Rectangle &rect, UiFlags flags, int spacing, int lineHeight, bool drawTextCursor)
{
	...
	for (; i < text.length(); i++) {
		...
		DrawArt(out, { characterPosition.x, characterPosition.y - lineHeight }, &Fonts[size], frame, 0, 0, false);
		if (text[i] != '\n')
